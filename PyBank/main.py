...
# open budget_data and read it in 
with open(csvpath) as csvfile:
    ...
    for row in csvreader:
        ...
        #now checks each p/l to see if its the highest profit
        if (pl_difference > max_profit): 
            max_profit = current_pl - last_pl
            max_month_name = row[0]

           # print(max_month_name)
        #wasn't the highest profit, so now checking to see if its the highest loss
    ...
    avg_change.pop(0) #replacing p/l change
